...
| `ZAPRET_CMD` | — | The [Zapret(nfqws)](https://github.com/bol-van/zapret) strategy; when set, the `ZAPRET` proxy outbound appears. The container includes .bin files for fakes from the [zapret-discord-youtube](https://github.com/Flowseal/zapret-discord-youtube) repository. The paths for using them are `@/zapret-fakebin/quic_initial_www_google_com.bin`, `@/zapret-fakebin/tls_clienthello_4pda_to.bin`, `@/zapret-fakebin/tls_clienthello_max_ru.bin`, `@/zapret-fakebin/tls_clienthello_www_google_com.bin` |
