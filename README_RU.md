...
| `SUB_LINKxx_PROXY`     | `DIRECT`                              | Указание через какой [прокси](https://wiki.metacubex.one/ru/config/proxy-providers/#proxy) запрашивать подписку. Пример `SUB_LINK1_PROXY` со значнием `GLOBAL` будет запрашивать подписку через прокси-группу `GLOBAL` |
