...
| `SUB_LINKxx_PROXY`     | `DIRECT`                              | Указание через какую [прокси-группу](https://wiki.metacubex.one/ru/config/proxy-groups) запрашивать подписку. [DOCs](https://wiki.metacubex.one/ru/config/proxy-providers/#proxy). Пример `SUB_LINK1_PROXY` со значнием `GLOBAL` будет запрашивать подписку через прокси-группу `GLOBAL` |
